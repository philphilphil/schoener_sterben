...
- `DRAFTS_PATH` points at the root of a notes tree (walked recursively). A file is treated as a draft when its frontmatter has `draft: true` and `tags` includes `oper`.
- In `dev`, the integration copies matching `.md` files into `src/content/drafts/` as `.mdx` (flattened to basename), keeps them in sync, and watches the source tree for changes.
...
- `utils/publish_post.sh <path/to/draft.md>` publishes a Markdown draft by full path. It validates required frontmatter (`title`, `slug`, `summary`, `date`), copies the draft into `src/content/posts/` as `<date>_<slug>.mdx`, and renames the source file in place to `<date>_<slug>.md` (in the same directory).
